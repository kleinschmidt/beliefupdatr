...
data {
  ...
  matrix[m,l] n[k];             // number of training observations
  matrix[m,l] xbar[k];          // mean of training observations
  matrix[m,l] xsd[k];           // sample standard deviation of training obs
  ...
  int block_test[n_test];       // block number for each test observation
...
transformed data {
  ...
  for (block in 1:k) {
    ss[block] = (n[block] - 1) .* xsd[block];
    block_max_n[block] = max(n[block]);
  }

  n_each = max(block_max_n);
...
parameters {
  ...
  real<lower=0, upper=1> lapse_rate[k]; // lapse rate by block
}

transformed parameters {
  // updated beliefs depend on input/group
  real mu_n[k,m,l];                 // updated expected mean
  real<lower=0> kappa_n[k,m,l];     // updated mean pseudocount
  real<lower=0> sigma_n[k,m,l];     // updated expected sd
  real<lower=0> nu_n[k,m,l];        // updated sd pseudocount
  real<lower=0> t_scale[k,m,l];     // scale parameter of predictive t distribution
  ...
  for (cat in 1:m) {
    for (group in 1:l) {
      for (block in 1:k) {
        kappa_n[block,cat,group] = kappa_0 + n[block,cat,group];
        nu_n[block,cat,group] = nu_0 + n[block,cat,group];
        mu_n[block,cat,group] = (mu_0[cat] * kappa_0 + xbar[block,cat,group] * n[block,cat,group]) / kappa_n[block,cat,group];
        sigma_n[block,cat,group] = sqrt((nu_0*sigma_0[cat]^2 +
                                         ss[block,cat,group] +
                                         (n[block,cat,group]*kappa_0)/(kappa_n[block,cat,group]) *
                                         (mu_0[cat] - xbar[block,cat,group])^2
                                         ) /
                                        nu_n[block,cat,group]);
        t_scale[block,cat,group] = sigma_n[block,cat,group] * sqrt((kappa_n[block,cat,group] + 1) / kappa_n[block,cat,group]);
      }
    }
  }
...
model {
  vector[m] lapsing_probs[k];

  for (block in 1:k) {
    lapsing_probs[block] = rep_vector(lapse_rate[block] / m, m);
  }
  ...
  for (i in 1:n_test) {
    int blck = block_test[i];
    z_test_counts[i] ~ multinomial(p_test_conj[i] * (1-lapse_rate[blck]) + lapsing_probs[blck]);
  }

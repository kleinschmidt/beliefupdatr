...
n_part <- 100
particles <- init_particles(p0, n_part)
...
ps <- reduce(xs[1:30], filter_fearnhead, .init=particles[1], N=length(particles))

```

What are the distributions that the particle filter infers for this data?

```{r, particle-post-pred, fig.width=9.75, fig.height=3.66}

x_pred <- seq(-8, 8, length.out=200)

d_predict_particles_marginal_df(x_pred, ps) %>%
  ggplot(aes(x=x, color=category)) +
  geom_histogram(data=data_frame(x=xs,
                                 category=rep(c('a', 'b'), length.out=length(x))),
                 aes(y=..density..*2), position='identity',
                 alpha=0.2, fill='black', color=NA) +
  geom_line(aes(y=pred))

# plot each of the particles individually
d_predict_particles_df(x_pred, ps) %>%
  ggplot(aes(x=x, color=category)) +
  geom_histogram(data=data_frame(x=xs,
                                 category=rep(c('a', 'b'), length.out=length(x))),
                 aes(y=..density..*2), position='identity',
                 alpha=0.2, fill='black', color=NA) +
  geom_line(aes(y=pred, group=paste(category, particle_id), alpha=weight)) +
  scale_alpha(range=c(0.05, 1))

```

From these plots we can see that a few particles dominate, and these are the
ones that correspond to predictive distributions that match the data well.  We
can see why by looking at how each particle (on the x axis) classifies the
training data (each row shows one trial), and the weight it's assigned by the
end (opacity, on a log-scale):

```{r, particle-classification, fig.width=8.2, fig.height=2.8}
...
```{r particle-class-linear, fig.width=8.2, fig.height=2.8}
ggplot(zs, aes(x=particle_id, y=x_id, fill=z, alpha=w)) +
  geom_tile() +
...
Kleinschmidt and Jaeger (2016) tested how an incremental belief updating model
could fit human behavior in unsupervised distributional learning.  However, they
made a simplifying assumption that belief updating itself was supervised (that
the mean and variance of each cluster was known to the subjects).

With a particle filter, we can back off on that assumption.

```{r load-dist-learn-data}

require(supunsup)

dat <- supunsup::supunsup_clean

# pull out one subject from each condition
dat_one <- dat %>%
  group_by(bvotCond) %>%
  filter(subject == first(subject))

ggplot(dat_one, aes(x=vot, fill=bvotCond)) +
...
# inferred prior from K&J(2016)

prior <- list(b = nix2_params(mu=-11, sigma2=19^2, kappa=243, nu=772),
              p = nix2_params(mu=56, sigma2=16^2, kappa=243, nu=772))
n_part <- 100
...
updated <- dat_one %>%
  nest() %>%
  mutate(updated_particles =
           map(data, ~ reduce(.$vot,
                        filter_fearnhead, 
                        .init=init_particles(prior, 1), 
                        N=n_part)))


reduce(dat_one$vot[1:222], filter_fearnhead, .init=init_particles(prior, 1), N=n_part)
...
updated_pred <- 
  updated %>%
  mutate(updated_pred = map(updated_particles,
                            d_predict_particles_marginal_df,
                            x = -20:70)) %>%
  unnest(updated_pred) %>%
  rename(vot=x)

ggplot(updated_pred, aes(x=vot, y=pred, linetype=category, color=bvotCond)) +
  geom_line()
...
updated_pred %>%
  ...
  filter(category == 'p') %>%
  ggplot(aes(x=vot, y=pred, color=bvotCond)) +
  geom_line() +
  geom_pointrange(data=dat, aes(y=respP, group=bvotCond), stat='summary', fun.data=mean_cl_boot)

```

```{r classification-training-data}

training_classes_df <- function(ps) {
  ps %>%
    transpose() %>%
    as_data_frame() %>%
    mutate(w = as_vector(w)) %>%
...
## how is training data classified?

updated %>% unnest(map(updated_particles, training_classes_df)) %>%
  ggplot(aes(x=x_id, y=particle_id, fill=z, alpha=w)) +
  geom_tile() +
  facet_wrap(~bvotCond)

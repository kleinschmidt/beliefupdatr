...
```{r compile}
mod <- beliefupdatr::model_filename('conj_id_lapsing_fit.stan') %>%
  rstan::stan_model()
```
...
```{r fit-model}
fit <- rstan::sampling(mod, data=d_stan_little, chains=4, iter=1000)
```
...
mod_inc <- model_filename("conj_id_lapsing_sufficient_stats_fit.stan") %>%
  rstan::stan_model() 
